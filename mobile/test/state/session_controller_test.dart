...
StoredSession _stored({DateTime? expiresAt}) => StoredSession(
      token: 'jwt',
      userId: 7,
      email: 'a@b.c',
      role: 'PATIENT',
...
void main() {
  ...
  test('expired stored token → cleared and logged out', () async {
  ...
  test('valid token accepted by /auth/me → session with fresh identity',
      () async {
    when(() => storage.read()).thenAnswer((_) async => _stored());
    when(() => authRepository.me()).thenAnswer((_) async =>
        const MeResponse(userId: 7, email: 'a@b.c', role: UserRole.patient));
  ...
  test('signIn persists the session and updates state', () async {
    when(() => storage.read()).thenAnswer((_) async => null);
